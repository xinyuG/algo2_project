...
exports.pointC = function(leftTurn){
    return lines("<strong class=blue>Point c</strong> appears on both ends of the deque because it was the last point added to the hull. This is an invariant we want to maintain. But the next point in the deque, in each direction, is also important.",
            "We'll say that <strong class=red>point " +
            (leftTurn ? "a" : "b" ) +
            "</strong> will be red because it appears to point c's left (L). Notice that this means it appears on the right side of the deque. Similarly, <strong class=green> point " +
            (leftTurn ? "b" : "a" ) +
            "</strong> will be green because it appears to point c's right(R).",
...
exports.rbpRegions = lines("Looking at those two outermost points on each side of the deque, we extend the lines they form to create regions, based on where point d could land.",
        "A sharp left turn puts us in the <strong class=red>red</strong> region. A left turn could violate the invariant on the right side of the deque, where we should be making a right turn.",
        "Similarly, a sharp right turn puts us in the <strong class=green>green</strong> region. And if we go into the <strong class=blue>blue</strong> region, we may need to worry about both ends of the deque.",
...
exports.yellowRegion = lines("But point d could also land within the known hull, requiring no changes to the deque. This is the <strong class=yellow>yellow</strong> region. If point d lands here, we simply discard it and wait for the next point.",
        ...
        "Notice how the four permissible regions meet at point c, the last point added to the hull (Pk). We can determine which region we land in from the turn direction of lines <em>acd</em> and <em>bcd</em>. Thus, the regions are illustrative only, and do not need to be calculated explicitly.",
...
exports.pointInGreen = lines("You've placed a point in the <strong class=green>green</strong> region. We now need to pop from the left side of the deqeue until reading leftwards once again corresponds to making only left turns."
        );

exports.pointInRed = lines("Since the point was placed in the red region, we now need to pop from the right side of the deqeue until reading rightwards once again corresponds to making only right turns."
        );

exports.pointInBlue = lines("You've placed a point in the <strong class=blue>blue</strong> region, which is just the composition of the red and blue regions!",
        "We pop from both sides of the deque (starting on the left) to restore the order invariant."
        );

exports.greenRight = lines("Since the point was placed in the green region, the right side of the deque does not need to be modified.",
        "In a real implementation, the algorithm would pop until these three points form a right turn when read rightward. The green region tells us humans that they already form a right turn.");

exports.redLeft = lines("You've placed a point in the <strong class=red>red</strong> region. This means that the left side of the deque does not need to be modified.",
